...
impl VisitorState {
    /// Core type resolution entry point
    pub fn get_or_create_type(&mut self, ty: &Type) -> TypeId {
        let type_str = ty.to_token_stream().to_string();
        if let Some(&id) = self.type_map.get(&type_str) {
            return id;
        }
    ...
    /// Detailed type analysis with relationship tracking
    pub fn process_type(&mut self, ty: &Type) -> (TypeKind, Vec<TypeId>) {
        let mut related_types = Vec::new();
        ...
        match ty {
            ...
            Type::Tuple(tuple) => {
                let elem_ids: Vec<TypeId> = tuple.elems.iter().map(|elem| self.get_or_create_type(elem)).collect();
            ...
            Type::Ptr(ptr) => {
                ...
                (TypeKind::RawPointer { is_mutable: ptr.mutability.is_some() }, vec![pointee_id])
            }
            Type::BareFn(bare_fn) => {
                ...
                (TypeKind::Function {
                    ...
                    abi: bare_fn.abi.as_ref().map(|abi| {
                        abi.name.as_ref().map_or("C".to_string(), |name| name.value())
                    }),
                }, related_ids)
            }
            Type::Paren(paren) => {
            ...
            Type::TraitObject(trait_obj) => {
                let bound_ids: Vec<TypeId> = trait_obj.bounds.iter().filter_map(|bound| {
                    if let TypeParamBound::Trait(trait_bound) = bound {
                        let bound_id = self.next_type_id();
                        self.code_graph.type_graph.push(TypeNode {
                            id: bound_id,
                            kind: TypeKind::Named {
                                path: trait_bound.path.segments.iter().map(|seg| seg.ident.to_string()).collect(),
                ...
                (TypeKind::TraitObject { dyn_token: trait_obj.dyn_token.is_some() }, bound_ids)
            }
            Type::ImplTrait(impl_trait) => {
                let bound_ids: Vec<TypeId> = impl_trait.bounds.iter().filter_map(|bound| {
                    if let TypeParamBound::Trait(trait_bound) = bound {
                        let bound_id = self.next_type_id();
                        self.code_graph.type_graph.push(TypeNode {
                            id: bound_id,
            ...
            _ => (
                TypeKind::Unknown { type_str: ty.to_token_stream().to_string() }, 
                Vec::new()
            )
        }
    }

    /// Handle trait bounds and lifetime constraints
    pub fn process_type_bound(&mut self, bound: &syn::TypeParamBound) -> TypeId {
        match bound {
            syn::TypeParamBound::Trait(trait_bound) => {
                let ty = Type::Path(TypePath {
                    qself: None,
    ...
    /// Extract lifetime identifiers
    pub fn process_lifetime_bound(&mut self, bound: &syn::Lifetime) -> String {
        bound.ident.to_string()
    }
}
