use crate::parser::nodes::{EnumNode, FieldNode, NodeId, StructNode, TypeDefNode, UnionNode, VariantNode, VisibilityKind};
...
use quote::ToTokens;
use syn::{Fields, FieldsNamed, FieldsUnnamed, Generics, Ident, Item, ItemEnum, ItemStruct, ItemUnion, Visibility};
...
pub trait StructVisitor: TypeProcessor {
    ...
    fn process_struct(&mut self, s: &ItemStruct) {
        ...
        let generic_params = self.process_generics(&s.generics);
        
        // Extract documentation and attributes
        let docstring = self.extract_docstring(&s.attrs);
        let attributes = self.extract_attributes(&s.attrs);
    ...
    fn process_enum(&mut self, e: &ItemEnum) {
        ...
        // Process generic parameters
        let generic_params = self.process_generics(&e.generics);
        
        // Extract documentation and attributes
        let docstring = self.extract_docstring(&e.attrs);
        let attributes = self.extract_attributes(&e.attrs);
    ...
    fn process_union(&mut self, u: &ItemUnion) {
        let union_id = self.next_node_id();
        let union_name = u.ident.to_string();
        
        // Process fields
        let fields = self.process_fields(&u.fields, union_id);
        
        // Process generic parameters
        let generic_params = self.process_generics(&u.generics);
        
        // Extract documentation and attributes
        let docstring = self.extract_docstring(&u.attrs);
        let attributes = self.extract_attributes(&u.attrs);
    ...
    fn process_fields(&mut self, fields: &Fields, parent_id: NodeId) -> Vec<FieldNode> {
        match fields {
            ...
            Fields::Unnamed(fields_unnamed) => self.process_unnamed_fields(fields_unnamed, parent_id),
            Fields::Unit => Vec::new(),
        }
    }
    ...
    fn process_named_fields(&mut self, fields: &FieldsNamed, parent_id: NodeId) -> Vec<FieldNode> {
        fields
            ...
            .iter()
            .map(|field| {
                let field_id = self.next_node_id();
                ...
                // Extract attributes
                let attributes = self.extract_attributes(&field.attrs);
    ...
    fn process_unnamed_fields(&mut self, fields: &FieldsUnnamed, parent_id: NodeId) -> Vec<FieldNode> {
        fields
            .unnamed
            .iter()
            .enumerate()
    ...
    /// Process enum variants
    fn process_variants(&mut self, variants: &syn::punctuated::Punctuated<syn::Variant, syn::Token![,]>, enum_id: NodeId) -> Vec<VariantNode> {
        variants
            .iter()
            .map(|variant| {
                let variant_id = self.next_node_id();
                ...
                // Extract attributes
                let attributes = self.extract_attributes(&variant.attrs);
