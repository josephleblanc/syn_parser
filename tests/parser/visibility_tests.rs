...
use syn_parser::parser::visitor::modules::CodeVisitor;
...
fn test_module_visibility() {
    ...
    let public_module = state.code_graph.modules.iter()
        .find(|m| m.name == "public_module")
        .unwrap();
    assert_eq!(public_module.visibility, VisibilityKind::Public);
    
    let private_module = state.code_graph.modules.iter()
        .find(|m| m.name == "private_module")
        .unwrap();
    assert_eq!(private_module.visibility, VisibilityKind::Restricted(vec!["super".to_string()]));
}
